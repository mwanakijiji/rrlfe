...
axes[0,1].imshow(ret.statistic.T, origin='lower', extent=(-3, 0, -3, 0), vmin=v_min, vmax=v_max)
axes[0,1].plot([-3, 0],[coeffs_liu[0]*(-3.0)+coeffs_liu[1],coeffs_liu[0]*(0.0)+coeffs_liu[1]], linestyle="-", color="gray", zorder=1)
axes[0,1].plot([-4, 1], [-4, 1], linestyle="--", color="gray")
...
colorbarobj = axes[1,1].imshow(ret.statistic.T, origin='lower', extent=(-3, 0, -3, 0), vmin=v_min, vmax=v_max) # plots + makes an object for the colorbar
...
axes[2,1].scatter(df_whitten["feh_whitten"], df_whitten["feh_rrlfe"], c = df_whitten["s_to_n"], cmap="viridis", vmin=v_min, vmax=v_max)
...
axes[2,1].plot([-4.0,1.0],[coeffs_whitten[0]*(-4.0)+coeffs_whitten[1],coeffs_whitten[0]*(1.0)+coeffs_whitten[1]], linestyle="-", color="grey", zorder=1)
axes[2,1].tick_params(axis='both', which='major', labelsize=char_size-2)
axes[2,1].annotate("S/N, Whitten+ 2021", xy=(-2.5, -3.75), xycoords='data', fontsize = char_size, color='k')
...
#axes[2,0].plot([-4, 1], [-4, 1],[coeffs_rrlfe_synth[0]*(-4.0)+coeffs_rrlfe_synth[1],coeffs_rrlfe_synth[0]*(1.0)+coeffs_rrlfe_synth[1]], linestyle="-", zorder=0, label="rrlfe synthetic basis set (PLACEHOLDER)")
axes[2,0].plot([-4, 1], [coeffs_single_epoch[0]*(-4.0)+coeffs_single_epoch[1],coeffs_single_epoch[0]*(1.0)+coeffs_single_epoch[1]], linestyle="-", zorder=0, label="SSPP, single-epoch")
axes[2,0].plot([-4, 1], [coeffs_coadded[0]*(-4.0)+coeffs_coadded[1],coeffs_coadded[0]*(1.0)+coeffs_coadded[1]], linestyle="-", zorder=0, label="SSPP, coadded")
axes[2,0].plot([-4, 1], [coeffs_liu[0]*(-4.0)+coeffs_liu[1],coeffs_liu[0]*(1.0)+coeffs_liu[1]], linestyle="-", zorder=0, label="Liu+ 2020")
axes[2,0].plot([-4, 1], [coeffs_lietal2023[0]*(-4.0)+coeffs_lietal2023[1],coeffs_lietal2023[0]*(1.0)+coeffs_lietal2023[1]], linestyle="-", zorder=0, label="Li+ 2023")
axes[2,0].plot([-4, 1], [coeffs_whitten[0]*(-4.0)+coeffs_whitten[1],coeffs_whitten[0]*(1.0)+coeffs_whitten[1]], linestyle="-", zorder=0, label="Whitten+ 2021")
