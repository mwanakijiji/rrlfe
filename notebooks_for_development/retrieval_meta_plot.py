...
df_sspp_coadded = pd.read_csv("comparison_sspp_coadded_20230820.csv") # source: examine_sspp_output.py
...
axes[0,1].annotate("       S/N, SSPP\n(single-epoch)", xy=(-1.55, -2.8), xycoords='data', fontsize = char_size, color='k')
...
coeffs_coadded = np.polyfit(df_sspp_coadded["feh_sspp_coadded"][idx_sane_coadded], df_sspp_coadded["feh_rrlfe"][idx_sane_coadded], w = df_sspp_coadded["s_to_n"][idx_sane_coadded], deg=1)
...
x_bins = np.linspace(-3, 0, 50)
y_bins = np.linspace(-3, 0, 50)
ret = binned_statistic_2d(df_sspp_coadded['feh_sspp_coadded'], df_sspp_coadded['feh_rrlfe'], df_sspp_coadded['s_to_n'], statistic=np.mean, bins=[x_bins, y_bins])
...
axes[1,1].annotate("S/N, SSPP\n(coadded)", xy=(-1.15, -2.8), xycoords='data', fontsize = char_size, color='k')
